...
export class FlowerRingSystem {
    ...
    // Rebuild ring so that each provided texture appears exactly once
    rebuildWithTextures(textures) {
        try {
            // Remove existing sprites from scene
            if (this.flowerRing) {
                this.flowers.forEach(f => {
                    if (f && this.flowerRing) this.flowerRing.remove(f);
                });
            }
            this.flowers = [];

            // Position sprites evenly on a circle
            const count = textures.length;
            const radius = 260; // smaller ring for fewer items
            const y = 0;
            for (let i = 0; i < count; i++) {
                const tex = textures[i];
                const material = this.getMaterialFromCache(tex);
                const sprite = new THREE.Sprite(material);
                const angle = (i / count) * Math.PI * 2;
                sprite.position.set(Math.cos(angle) * radius, y, Math.sin(angle) * radius);
                const size = 22; // consistent size for showcase
                sprite.scale.set(size, size, 1);
                sprite.lookAt(0, y, 0);
                this.flowers.push(sprite);
                if (this.flowerRing) this.flowerRing.add(sprite);
            }

            // Save current textures for later tweaks
            this.currentTextures = [...textures];

            // Cleanup cache of unused materials if any
            this.cleanupMaterialCache();
        } catch (error) {
            console.error('❌ Error rebuilding ring with exact textures:', error);
        }
    }

    // Load multiple data URLs and show exactly one sprite per image
    setExactTexturesByDataURLs(dataURLs) {
        const loader = new THREE.TextureLoader();
        const loadPromises = dataURLs.map(url => {
            return new Promise((resolve) => {
                loader.load(
                    ...
                    (texture) => {
                        try {
                            // Process canvas (EXIF, rounded corners) similar to other flows
                            const canvas = this.getCanvasFromPool();
                            const ctx = canvas.getContext('2d');
                            canvas.width = texture.image.width;
                            canvas.height = texture.image.height;
                            ...
                            console.error('Lỗi xử lý texture (exact):', error, url);
                            resolve(this.createFallbackTexture());
                        }
                    },
                    undefined,
                    (error) => {
                        console.error('Lỗi load texture (exact):', error, url);
                        resolve(this.createFallbackTexture());
                    }
                );
            });
        ...
        Promise.all(loadPromises).then(textures => {
            const valid = textures.filter(t => t);
            if (valid.length === 0) {
                console.warn('⚠️ No valid textures, using fallback');
                this.rebuildWithTextures([this.createFallbackTexture()]);
                return;
            }
            // Dispose previous textures before rebuild
            this.disposeTextures();
            this.rebuildWithTextures(valid);
        });
    }
